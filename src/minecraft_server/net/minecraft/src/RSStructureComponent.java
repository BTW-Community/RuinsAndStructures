...
abstract class RSStructureComponent extends StructureComponent {
	protected RSStructureComponent(int par1) {
		super(par1);
	}
	...
	protected int getMetadataWithOffset(int blockId, int meta)
	{
		/*try {*/
			...
			if (blockId == Block.wood.blockID || (RSDecoIntegration.isDecoInstalled() && (blockId == RSDecoIntegration.barkLogStripped.blockID || blockId == RSDecoIntegration.strippedLog.blockID))) {
				if (this.coordBaseMode == 1 || this.coordBaseMode == 3) {
					if ((meta & 8) != 0) {
						return meta - 4;
					}
